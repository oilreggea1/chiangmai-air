...
for (const dir of SCAN_DIRS) {
  for (const file of walk(join(ROOT, dir))) {
    ...
    lines.forEach((line, i) => {
      ...
      // ตัวเลขที่ตามด้วยหน่วยเงิน = ราคาแน่นอน
      //
      // เคยหลุดมาแล้ว 19 ส.ค. 2569: หน้าภาษาจีนเขียน "999 泰铢起" กับ "1,299 泰铢起"
      // ไว้ตรง ๆ ทั้งที่ค่าเหล่านี้มีอยู่ใน p.washer แล้ว ตัวดักจับไม่ได้เพราะรู้จัก
      // แค่ "บาท" "THB" ".-" ไม่รู้จัก 泰铢 ซึ่งเป็นคำว่าบาทในภาษาจีน
      // ตอนนั้นตัวเลขบังเอิญยังตรง แต่ถ้าเจ้าของปรับราคาเมื่อไร หน้าจีนจะค้างเงียบ ๆ
      // เวลาเพิ่มภาษาใหม่ ต้องเติมหน่วยเงินของภาษานั้นเข้ามาที่บรรทัดนี้ด้วยเสมอ
      for (const m of line.matchAll(/([\d,]+)\s*(?:บาท|\.-|THB|泰铢|铢)/g)) {
        const n = m[1];
