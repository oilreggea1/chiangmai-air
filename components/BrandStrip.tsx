...
export function BrandStrip({
  ...
  note = "ผมรับงานล้าง ซ่อม ติดตั้ง และย้ายแอร์ทุกยี่ห้อที่จำหน่ายในไทย โดยไม่ได้เป็นตัวแทนจำหน่ายหรือศูนย์บริการของแบรนด์ใด เครื่องหมายการค้าทั้งหมดเป็นของเจ้าของแบรนด์นั้น ๆ",
}: {
  tone?: "dark" | "light";
  title?: string;
  note?: string;
        ...
        {brands.map((b) => (
          <li key={b.name} className="flex h-8 items-center justify-center sm:h-7">
            {b.logo ? (
              ...
              <span
                className={`text-center text-xs leading-snug font-bold tracking-wide ${label}`}
              >
                {b.name}
              </span>
            )}
